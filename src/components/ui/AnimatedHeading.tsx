  ...
  return (
    ...
    >
      {[row1, row2].map((row, rowIdx) => (
        <div key={rowIdx} className="flex flex-wrap items-baseline justify-center gap-x-4 gap-y-1 md:gap-x-5">
          {row.map((word, i) => (
            <motion.span
              key={`${rowIdx}-${i}`}
              variants={wordCloudWord}
              className={`font-display text-[clamp(36px,6vw,80px)] leading-[1.1] ${
                isAccent(word)
